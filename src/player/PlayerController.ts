...
export class PlayerController {
  ...
  private _hRay = new THREE.Raycaster();
  private _collisionMeshes: THREE.Object3D[] = [];
  collisionRadius = 0.45; // 水平碰撞半径
  private _collisionDirs: THREE.Vector3[] = []; // 预计算水平方向
  maxSubStep = 0.02; // 物理子步长上限
  maxIterations = 2; // 每帧额外迭代（用于连续碰撞修正）
  enableVelocitySweep = true; // 速度方向预扫
  sweepSkin = 0.02; // 与命中面保持的最小皮肤距离
  private _lastSafePos = new THREE.Vector3();
  private _tmp = new THREE.Vector3();
  collisionSegments = 24; // 水平方向细分数量（越多越精确）
  stepHeight = 0.3; // 台阶高度
  enableStepUp = true;

  constructor(camera: THREE.PerspectiveCamera, worldRoots: THREE.Object3D[]) {
    this.camera = camera;
  ...
  private _step(dt: number) {
    ...
    this._downRay.set(
      this.position.clone().addScaledVector(up, 0.2),
    ...
    const groundSet = this._collisionMeshes.length
      ...
      : (() => {
          ...
          for (const r of this.worldRoots) {
            r.traverse((o: THREE.Object3D) => {
              if ((o as any).isMesh) tmp.push(o);
            });
          }
          return tmp;
        })();
          ...
          .add(
            new THREE.Vector3(
              ...
              this.collisionHeight * 0.5 -
                (this.eyeHeight - this.collisionHeight),
              0
            )
          );
        this._hRay.set(mid, dir);
        this._hRay.far = speed2D * dt + this.collisionRadius;
        const sweepHits = this._hRay.intersectObjects(
          this._collisionMeshes,
          false
        );
        if (sweepHits.length) {
    ...
    if (this._collisionMeshes.length) {
        ...
        for (const h of ringHeights) {
          const ringCenter = this.position
            ...
            .add(
              new THREE.Vector3(
                0,
                h - (this.eyeHeight - this.collisionHeight),
                0
              )
            );
          for (const d of this._collisionDirs) {
            this._hRay.set(
              ringCenter.clone().addScaledVector(d, -radius * 0.72),
              d
            );
            this._hRay.far = radius + 0.18;
            const hit = this._hRay.intersectObjects(
              this._collisionMeshes,
              false
            );
            if (hit.length) {
      ...
      if (totalPushLen > radius * 0.6) {
        ...
        for (const d of this._collisionDirs) {
          this._hRay.set(
            testCenter.clone().addScaledVector(d, -radius * 0.75),
            d
          );
          this._hRay.far = radius + 0.15;
          const htest = this._hRay.intersectObjects(
            this._collisionMeshes,
            false
          );
  ...
  setCollisionMeshes(meshes: THREE.Object3D[]) {
